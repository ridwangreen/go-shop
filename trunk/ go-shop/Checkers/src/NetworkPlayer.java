...
public class NetworkPlayer extends Player {
 ...
 //   The commands that can be send back and forth
   private final int RESIGN = 0;
   private final int DRAWOFFER = 1;
   private final int ACCEPTDRAW = 2;
   private final int REFUSEDRAW = 3;
   private final int ENDOFGAME = 4;
   private final int ROGER = 90;
   private final int RESEND = 98;
   private final int NETWORKPLAYER = 1;

   // The port number we will read from
   private final int PORTNUM = 1051;
    ...
    */
   public NetworkPlayer( int playerNum, Driver theDriver ) {

    	// call super classes (player) constructor to give it starting info
    	super( playerNum, theDriver );
   ...
   public void waitForPlayer()
   {
       ...
       do {
		...
		NetworkMove remoteMove = ( NetworkMove ) inputObj;
                ...
                theDriver.makeMove( new Move( getColor(), 
   ...
   /**
    * Inform the remote player that their draw offer was refused
    */
   public void declineDraw() {
	   // send the draw offer over the network
       try {
           out.writeObject( new Integer( REFUSEDRAW ) );
           
       } catch ( IOException e ) {
           System.err.println( "IOException while sending draw refusal over"
                               + " network stream.");
       }
   }
