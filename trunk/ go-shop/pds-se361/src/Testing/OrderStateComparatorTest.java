/*
 * Filename:
 *		OrderStateComparatorTest.java
 *
 * Version:
 *	   $Id: OrderStateComparatorTest.java 406 2011-02-14 03:23:22Z Brian Baum $
 *
 * Revision:
 *	   $Log$
 */
 ...
 *		Jonathan Olin
 */
public class OrderStateComparatorTest {
	
	private OrderStateComparator osc;
	...
	public void setUp() throws Exception {
		...
		o3 = new Order(3, "5553338282", "RIT", "Ned", cV);
	...
	@Test
	public void testOrderStateEqual() {
		for (States s : States.values()) {
			o1.setCurrentState( s );
			o2.setCurrentState( s );
			assertTrue(osc.compare( o1, o2 ) ==0);
		}
	}
	
	@Test
	public void testOrderStateGreater() {
		for (int i=1; i <= 4; i++) {
			for (int j=0; j<i; j++) {
				o1.setCurrentState( States.values()[i] );
				o2.setCurrentState( States.values()[j] );
				assertTrue(osc.compare( o1, o2 ) < 0);
			}
		}
	}
	...
	@Test
	public void testOrderStateLess() {
		for (int i=3; i>=0; i--) {
			for (int j=4; j>i; j--) {
				o1.setCurrentState( States.values()[i] );
				o2.setCurrentState( States.values()[j] );
